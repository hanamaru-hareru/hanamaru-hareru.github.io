...
function calender_show_details(day_id) {
    for(let i=1; i<32; ++i) {
        ...
        if(day_id === i) {
            ...
            day_button.classList.add('calender-select');
        } else {
            day_detail.setAttribute('hidden', 'true');
            day_button.classList.remove('calender-select');
        }
    }
}
...
function calender_render(year, month) {
    ...
    document.getElementById('month-display').innerHTML = month;
    // Update month URL.
    for(let i=1; i<13; ++i) {
        document.getElementById('month-item-'+i).setAttribute('href', '#calender&year='+year+'&month='+i);
    }
...
function calender_init_ui() {
    ...
    for(let i=0; i<years.length-1; ++i) {
        year_items.push('<li><a class="dropdown-item" href="#calender&year='+years[i]+'&month=1">'+years[i]+'</a></li>');
    }
    // Or the 2019, we add the month when she first live.
    year_items.push('<li><a class="dropdown-item" href="#calender&year=2019&month=8">2019</a></li>');
    document.getElementById('year-list').innerHTML = year_items.join('\n');
    // Render the calender header.
